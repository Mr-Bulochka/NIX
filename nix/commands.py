...
@register("help", "show this help", "/help")
...
def cmd_attempts(app: "NixApp", args: list[str]) -> CommandResult:
    if not args:
        app.ui.show_message("SYSTEM",
                            f"attempts: {app.config.attempts}/{app.config.max_attempts}")
        return CommandResult()
    token = args[0]
    try:
        if token.startswith(("+", "-")):
        ...
        app.ui.show_message("ERROR", "usage: /attempts N | /attempts +N | /attempts -N")
    return CommandResult()
...
def cmd_mode(app: "NixApp", args: list[str]) -> CommandResult:
    if not args:
        app.ui.show_message("SYSTEM", f"mode: {app.config.mode}")
        return CommandResult()
    new_mode = args[0].lower()
    from .config import VALID_MODES
    if new_mode not in VALID_MODES:
        app.ui.show_message("ERROR", f"valid modes: {', '.join(VALID_MODES)}")
        return CommandResult()
    app.config.mode = new_mode
    app.config_store.save(app.config)
    app.ui.show_message("SYSTEM", f"mode set to: {new_mode}")
...
@register("clear", "redraw the interface", "/clear")
...
def cmd_version(app: "NixApp", args: list[str]) -> CommandResult:
    ...
    app.ui.show_message("SYSTEM", f"NIX v{__version__}")
    return CommandResult()
